...
//
// prioritaet (P11, Plan-Abschnitt B3): für das jeweils größte/wichtigste Bild
// einer Seite (LCP-Kandidat) – setzt fetchpriority="high", loading="eager"
// und decoding="sync" statt der sonst verzögerten Standardwerte, damit der
// Browser das Bild sofort und mit hoher Priorität lädt.

export function bild({ pfad, daten, name, alt, sizes = "100vw", klasse = "", eager = false, prioritaet = false }) {
  ...
  const ladeAttr = prioritaet ? "eager" : (eager ? "eager" : "lazy");
  const decodingAttr = prioritaet ? "sync" : "async";
  const fetchpriorityAttr = prioritaet ? ` fetchpriority="high"` : "";

  return `<picture${klasseAttr}>
  <source type="image/avif" srcset="${srcset("avif")}" sizes="${sizes}">
  <source type="image/webp" srcset="${srcset("webp")}" sizes="${sizes}">
  <img src="${hauptbild}" srcset="${srcset("jpg")}" sizes="${sizes}" width="${eintrag.breite}" height="${eintrag.hoehe}" alt="${alt ?? ""}"${fetchpriorityAttr} loading="${ladeAttr}" decoding="${decodingAttr}">
</picture>`;
}
